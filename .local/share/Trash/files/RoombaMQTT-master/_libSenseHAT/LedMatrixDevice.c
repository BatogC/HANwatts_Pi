...
static void unpack_pixel(uint16_t pixel, uint8_t *r, uint8_t *g,
                         uint8_t *b)
{
   *r = (pixel & 0xF800) >> 8;
   *g = (pixel & 0x07E0) >> 3;
...
int init_fb(void)
{
   ...
   struct dirent *dent = (struct dirent *)malloc(
   ...
   while (1) {
      ...
      if (strncmp("fb", dent->d_name, 2) == 0) {
         char path[PATH_MAX];
         snprintf(path, PATH_MAX, "/sys/class/graphics/%s/name",
                  dent->d_name);
         FILE *f = fopen(path, "r");
         if (f == NULL)
            continue;
...
int set_pixel(int x, int y, uint8_t r, uint8_t g, uint8_t b)
{
   ...
   if (pwrite(fbfd, &pixel, sizeof(pixel), (x + y * 8) * 2) !=
       sizeof(pixel)) {
      return errno;
   }
   return 0;
...
int get_pixel(int x, int y, uint8_t *r, uint8_t *g, uint8_t *b)
{
   ...
   uint16_t pixel;
   if (pread(fbfd, &pixel, sizeof(pixel), (x + y * 8) * 2) !=
       sizeof(pixel)) {
      return errno;
   }
   // fprintf(stderr, "p = %x\n", pixel);
