...
284     
285    \textcolor{comment}{#for readable timestamp use this at end of INSERT: time.strftime('%Y-%m-%d %T',
...
Definition at line 290 of file S\+Q\+Lfunction.\+py.


\begin{DoxyCode}
290 \textcolor{keyword}{def }\hyperlink{namespace_s_q_lfunction_ab58c55eebaf8990eb9c810968db1dc50}{old\_photonMeasure\_callback}(client, userdata, message):
291     con = lite.connect(path)
292     cur = con.cursor()
293     data = message.payload    
294     data = data.decode(\textcolor{stringliteral}{'UTF-8'})
295     print(data)
296     
297     index = []
298     \textcolor{keywordflow}{for} i \textcolor{keywordflow}{in} range(len(data)):
299         \textcolor{keywordflow}{if} (data[i] == \textcolor{stringliteral}{'%'}):
300             index.append(i)
...
313     cur.execute(\textcolor{stringliteral}{"INSERT INTO photonMeasure(UIDtag, SocketID, V1, V2, V3, I1, I2, I3, P, E, F, Time)
       VALUES(?,?,?,?,?,?,?,?,?,?,?,?)"},
314                 (UserID, SocketID, V1, V2, V3, I1, I2, I3, P, E, F, Time))
315     con.commit()
316     \textcolor{comment}{#print(V1)}
317 
\end{DoxyCode}
...
Definition at line 80 of file S\+Q\+Lfunction.\+py.
...
\begin{DoxyCode}
80 \textcolor{keyword}{def }\hyperlink{namespace_s_q_lfunction_ab3031ccbbdb3f5a04bd7d66ceb4494c7}{on\_disconnect}(client, userdata, rc):
81     client.connected\_flag = \textcolor{keyword}{False}
82     \textcolor{keywordflow}{if} rc != 0:
83         print(\textcolor{stringliteral}{"Unexpected disconnection."})
84         client.bad\_connection\_flag = \textcolor{keyword}{True}
85     \textcolor{keywordflow}{else}:
86         print(\textcolor{stringliteral}{"Normal disconnection."})
87 
88 \textcolor{comment}{# def SendUser\_callback(client, userdata, message):}
89 \textcolor{comment}{#     #print(message.payload)}
90 \textcolor{comment}{#     con = lite.connect(path)}
91 \textcolor{comment}{#     cur = con.cursor()}
92 \textcolor{comment}{#     cur.execute('select * from list')}
93 \textcolor{comment}{# }
94 \textcolor{comment}{#     data = cur.fetchall()}
95 \textcolor{comment}{#     dataSend = ""}
96 \textcolor{comment}{# }
97 \textcolor{comment}{#     for element in data:}
98 \textcolor{comment}{#         print(element)}
99 \textcolor{comment}{#         dataSend +=
       (str(element[0])+'%'+element[1]+'%'+element[2]+'%'+str(element[3])+'%'+element[4]+'%'+str(element[5])+'%'+element[6]+'%'+str(element[7])+'%'+str(element[8])+'%')}
100 \textcolor{comment}{#     }
101 \textcolor{comment}{#     client.publish("HANevse/UserList", dataSend, 2, True)}
102 \textcolor{comment}{#     #publish.single("HANevse/UserList", dataSend, hostname=broker)}
103 \textcolor{comment}{#     #print(dataSend)}
104 
\end{DoxyCode}
...
168             \textcolor{keywordflow}{else}: \textcolor{comment}{#if swiped less than 20s ago}
169                 \textcolor{keywordflow}{if} (socketUsed == socketId): \textcolor{comment}{#if this socket is used now}
170                     \textcolor{keywordflow}{if} (socketId == dataUser[1]): \textcolor{comment}{# if user already uses this socket}
171                         dataSend += \textcolor{stringliteral}{"5"} \textcolor{comment}{# you just started using this socket less than 20s ago}
172                     \textcolor{keywordflow}{else}:
173                         dataSend += \textcolor{stringliteral}{"3"} \textcolor{comment}{# socket is occupied by another user}
174                 \textcolor{keywordflow}{else}: \textcolor{comment}{#if this socket is free}
175                     \textcolor{keywordflow}{if} dataUser[1] \textcolor{keywordflow}{is} \textcolor{keywordtype}{None}: \textcolor{comment}{#if user was not using any socket}
176                         dataSend += \textcolor{stringliteral}{"2"} \textcolor{comment}{#charger is free, but you already swiped less than 20s ago}
177                     \textcolor{keywordflow}{else}:
178                         dataSend += \textcolor{stringliteral}{"6"}  \textcolor{comment}{# user already at another socket}
179         \textcolor{keywordflow}{else}:
180              dataSend += \textcolor{stringliteral}{"7"} \textcolor{comment}{#user not verified by admin           }
181     \textcolor{keywordflow}{else}:
182         dataSend += \textcolor{stringliteral}{"8"} \textcolor{comment}{#user not in the userlist}
183     \textcolor{keywordflow}{try}:
184         con.commit()
185     \textcolor{keywordflow}{except} Exception \textcolor{keyword}{as} e:
186         \textcolor{keywordflow}{print} (e)
187     con\_local.commit()
188     
189     client.publish(\textcolor{stringliteral}{"HANevse/allowUser"}, dataSend, 0, \textcolor{keyword}{False})
190 
191 \textcolor{comment}{# def Update\_callback(client, userdata, message):}
192 \textcolor{comment}{#     con = lite.connect(path)}
193 \textcolor{comment}{#     cur = con.cursor()}
194 \textcolor{comment}{#     data = message.payload}
195 \textcolor{comment}{#     index = []}
196 \textcolor{comment}{#     for i in range(len(data)):}
197 \textcolor{comment}{#         if (data[i] == '%'):}
198 \textcolor{comment}{#             index.append(i)}
199 \textcolor{comment}{#     UserId = int(data[:index[0]])}
200 \textcolor{comment}{#     PendingCharger = int(data[index[0]+1:index[1]])}
201 \textcolor{comment}{#     StartTime = int(data[index[1]+1:index[2]])}
202 \textcolor{comment}{#     cur.execute("UPDATE list SET PendingCharger=? WHERE Id=?", (PendingCharger, UserId))}
203 \textcolor{comment}{#     cur.execute("UPDATE list SET StartTime=? WHERE Id=?", (StartTime, UserId))}
204 \textcolor{comment}{#     con.commmit()}
205 
206 
\end{DoxyCode}
...
Definition at line 35 of file S\+Q\+Lfunction.\+py.
...
Definition at line 418 of file S\+Q\+Lfunction.\+py.
...
Definition at line 55 of file S\+Q\+Lfunction.\+py.
...
Definition at line 14 of file S\+Q\+Lfunction.\+py.
...
Definition at line 33 of file S\+Q\+Lfunction.\+py.
...
Definition at line 420 of file S\+Q\+Lfunction.\+py.
